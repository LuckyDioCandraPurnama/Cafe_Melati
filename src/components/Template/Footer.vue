<template>
    <!-- Footer -->
            <footer class="sticky-footer bg-gray-200">
                <div class="container my-auto">
              <div class="row">
                <div class="col-lg-6">
                  <h2 class="h3 text-gray-900 font-weight-bold">Cafe Melati</h2>
                  <p class="mb-0 text-gray-700">
                    Aplikasi ini buat cafe melati <br />Pelanggan bisa memilih tempat dan 
                    <br />Bisa pesan makanan, minuman atau snack <br />Dan melakukan pembayaran melalui aplikasi
                  </p>
                </div>
                <div class="col">
                  <h2 class="h3 text-gray-900 font-weight-bold">Contact</h2>
                  <i class="far fa-envelope text-gray-700"> cafemelati@gmail.com</i><br />
                  <i class="fab fa-facebook-square text-gray-700"> Cafe Melati</i><br />
                  <i class="fab fa-instagram text-gray-700"> Cafe_Melati</i><br />
                  <p></p>
                </div>
              </div>
              <hr>
                    <div class="copyright text-center my-auto text-gray-900">
                        <span>Copyright &copy; Bimasakti 2022</span>
                    </div>
                </div>
                  <!-- <h6 class="m-0 font-weight-bold text-primary">About</h6> -->
            <!-- Card Body -->
            <!-- <div class="card-body"> -->
            <!-- </div> -->
            </footer>
            <!-- End of Footer -->
</template>